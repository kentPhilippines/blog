...
<div class="ne-news-detail">
    ...
    <nav class="ne-breadcrumb" aria-label="面包屑导航">
        <div class="ne-breadcrumb-container">
            <ol class="ne-breadcrumb-list">
                <li class="ne-breadcrumb-item">
                    <a href="/" class="ne-breadcrumb-link">
                        <i class="fas fa-home"></i>
                        <span>首页</span>
                    </a>
                </li>
                <?php if (!empty($news['categoryName'])): ?>
                    <li class="ne-breadcrumb-separator">/</li>
                    <li class="ne-breadcrumb-item">
                        <a href="/category.php?name=<?php echo urlencode(Utils::categoryToSlug($news['categoryName'])); ?>" class="ne-breadcrumb-link">
                            <?php echo htmlspecialchars($news['categoryName']); ?>
                        </a>
                    </li>
                <?php endif; ?>
                <li class="ne-breadcrumb-separator">/</li>
                <li class="ne-breadcrumb-item ne-breadcrumb-current" aria-current="page">
                    <span><?php echo htmlspecialchars(mb_substr($news['title'] ?? '', 0, 30)); ?><?php echo mb_strlen($news['title'] ?? '') > 30 ? '...' : ''; ?></span>
                </li>
            </ol>
        </div>
    </nav>
        ...
        <article class="ne-article">
            ...
            <div class="ne-article-header">
                ...
                <div class="ne-article-meta">
                    ...
                    // 显示主关键词（从域名配置的关键词中选择）
                    $mainKeyword = '';
                    if (defined('DYNAMIC_SITE_KEYWORDS') && !empty(DYNAMIC_SITE_KEYWORDS)) {
                        // 从域名配置的关键词中提取第一个作为主关键词
                        $domainKeywords = explode(',', DYNAMIC_SITE_KEYWORDS);
                        $mainKeyword = trim($domainKeywords[0]);
                    } elseif (!empty($news['keywords'])) {
                        // 如果域名没有配置关键词，则使用新闻的关键词
                        $keywords = explode(',', $news['keywords']);
                        $mainKeyword = trim($keywords[0]);
                    } elseif (!empty($news['categoryName'])) {
                        // 最后使用分类名作为主关键词
                        $mainKeyword = $news['categoryName'];
                    }
                    ?>
                    <?php if (!empty($mainKeyword)): ?>
                        <span class="ne-article-main-keyword">
                            <i class="fas fa-key"></i>
                            <?php echo htmlspecialchars($mainKeyword); ?>
                        </span>
                    <?php endif; ?>
                    <?php if (!empty($news['keywords'])): ?>
                        <span class="ne-article-keywords">
                            <i class="fas fa-tags"></i>
                            <?php echo htmlspecialchars($news['keywords']); ?>
                        </span>
                    <?php endif; ?>
                </div>
            </div>
    ...
    <div class="ne-comments-section">
        <div class="ne-comments" id="ne-comments">
            <h2 class="ne-section-title">评论</h2>
...
<style>
    ...
    .ne-breadcrumb-link:hover {
        color: #1a73e8;
        background: rgba(26, 115, 232, 0.1);
    }

    .ne-breadcrumb-link i {
        margin-right: 4px;
        font-size: 12px;
    }

    .ne-breadcrumb-separator {
        color: #999;
        margin: 0 8px;
        font-size: 12px;
    }
    ...
    .ne-article-main-keyword:hover {
        transform: translateY(-1px);
        box-shadow: 0 4px 12px rgba(102, 126, 234, 0.4);
    }

    .ne-article-main-keyword i {
        color: #fff;
        margin-right: 6px;
        font-size: 12px;
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.loadComments = async function(page) {
            ...
            try {
                ...
                const response = await fetch(`/api_proxy.html?action=comments&newsId=${newsId}&page=${page}`, {
                    method: 'GET',
                    headers: {
                ...
                if (response.ok && result.code === 200) {
                    ...
                    // 滚动到评论区顶部
                    document.querySelector('.ne-comments-section').scrollIntoView({
                        behavior: 'smooth'
                    });
                } else {
        ...
        window.handleSubmitComment = async function(event) {
            ...
            try {
                const response = await fetch('/api_proxy.html', {
                    method: 'POST',
                    headers: {
